 ...
 * @LastEditTime: 2025-05-18 17:43:25
...
void LogFormatter::init() {
  ...
  while (i < m_pattern.size()) {
    ...
    if (c == "%") {
      ...
      if (
        parsing_string) {   // 持续解析常规字符直到遇到%，解析出的字符串作为一个常规字符串加入patterns
...
Logger::Logger(const std::string& name)
  ...
  , m_createTime(util::GetElapsedMS()) {
}
    ...
    if (n["appenders"].IsDefined()) {
      for (size_t i = 0; i < n["appenders"].size(); i++) {
        ...
        if (type == "FileLogAppender") {
          lad.type = 1;
          if (!a["file"].IsDefined()) {
            std::cout << "log appender config error: file appender file is null, " << a
                      << std::endl;
            continue;
          }
