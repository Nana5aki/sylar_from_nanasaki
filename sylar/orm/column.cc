...
std::string Column::getSQLite3Default() {
  if (m_dtype <= TYPE_UINT64) {
    return m_default.empty() ? "0" : m_default;
  }

  // 字符串和二进制类型默认值处理
  if (m_dtype <= TYPE_BLOB) {
    return m_default.empty() ? "''" : "'" + m_default + "'";
  }

  // 时间戳类型默认值处理
  return m_default.empty() ? "'1980-01-01 00:00:00'" : m_default;
}
...
bool Column::init(const tinyxml2::XMLElement& node) {
  ...
  m_length = node.IntAttribute("length", 0);
