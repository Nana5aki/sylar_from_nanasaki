...
int SQLite3Stmt::bindTime(int idx, const time_t& value) {
  return bind(idx, sylar::util::Time2Str(value));
}
...
int SQLite3Stmt::bind(int idx, const char* value, Type type) {
  return sqlite3_bind_text(
    m_stmt, idx, value, strlen(value), type == COPY ? SQLITE_TRANSIENT : SQLITE_STATIC);
}

int SQLite3Stmt::bind(int idx, const void* value, int len, Type type) {
  return sqlite3_bind_blob(
    m_stmt, idx, value, len, type == COPY ? SQLITE_TRANSIENT : SQLITE_STATIC);
}

int SQLite3Stmt::bind(int idx, const std::string& value, Type type) {
  return sqlite3_bind_text(
    m_stmt, idx, value.c_str(), value.size(), type == COPY ? SQLITE_TRANSIENT : SQLITE_STATIC);
}
...
time_t SQLite3Data::getTime(int idx) {
  ...
  return sylar::util::Str2Time(str.c_str());
}
...
SQLite3::ptr SQLite3Manager::get(const std::string& name) {
  ...
  lock.unlock();
  std::string path = sylar::util::GetParamValue<std::string>(args, "path");
  if (path.empty()) {
  ...
  std::string sql = sylar::util::GetParamValue<std::string>(args, "sql");
  if (!sql.empty()) {
    if (rt->execute(sql)) {
