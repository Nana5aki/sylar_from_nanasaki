 ...
 * @LastEditTime: 2024-12-21 19:49:33
 * @FilePath: /MySylar/tests/test_config.cpp
 */
...
sylar::ConfigVar<std::unordered_set<int>>::ptr g_int_unordered_set = sylar::Config::Lookup(
    "global.int_unordered_set", std::unordered_set<int>{1, 2, 3}, "global int unordered_set");

sylar::ConfigVar<std::map<std::string, int>>::ptr g_map_string2int = sylar::Config::Lookup(
    "global.map_string2int", std::map<std::string, int>{{"key1", 1}, {"key2", 2}},
    "global map string2int");

sylar::ConfigVar<std::unordered_map<std::string, int>>::ptr g_unordered_map_string2int =
    sylar::Config::Lookup("global.unordered_map_string2int",
...
void test_config() {
    ...
    SYLAR_LOG_INFO(g_logger) << "g_int_vec value: "
                             << formatArray<std::vector<int>>(g_int_vec->getValue());
    SYLAR_LOG_INFO(g_logger) << "g_int_list value: "
                             << formatArray<std::list<int>>(g_int_list->getValue());
    SYLAR_LOG_INFO(g_logger) << "g_int_set value: "
...
int main(int argc, char* argv[]) {
    ...
    g_int->addListener([](const int& old_value, const int& new_value) {
        SYLAR_LOG_INFO(g_logger) << "g_int value changed, old_value: " << old_value
                                 << ", new_value: " << new_value;
    });
