 ...
 * @FilePath: /sylar_from_nanasaki/tests/test_fiber.cpp
 */
...
void test_fiber() {
  ...
  SYLAR_LOG_INFO(g_logger) << "fiber status: " << fiber->getState();   // READY、0
...
int main(int argc, char* argv[]) {
  ...
  for (int i = 0; i < 2; i++) {
    thrs.push_back(
      sylar::Thread::ptr(new sylar::Thread(&test_fiber, "thread_" + std::to_string(i))));
  }
